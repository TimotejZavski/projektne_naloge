...
if (
  ...
  // eslint-disable-next-line no-console
  console.error(
    "[FATAL] JWT_ACCESS_SECRET in JWT_REFRESH_SECRET morata biti razlicna.",
  );
  process.exit(1);
}
...
["JWT_ACCESS_SECRET", "JWT_REFRESH_SECRET"].forEach((key) => {
  ...
  if (val && val.length < 32 && process.env.NODE_ENV === "production") {
    // eslint-disable-next-line no-console
    console.warn(
      `[WARN] ${key} je krajsi od 32 znakov - to je nevarno za produkcijo.`,
    );
  }
});
...
module.exports = {
  ...
  RATE_LIMIT_REGISTER_MAX: toInt(process.env.RATE_LIMIT_REGISTER_MAX, 10),
  RATE_LIMIT_REGISTER_WINDOW_MS: toInt(
